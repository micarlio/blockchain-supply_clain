...
const OPCOES_DIFICULDADE = [0, 1, 2, 3, 4, 5, 6]

export function SeletorPowGlobal({ valor, pendente, aoSelecionar }: Props) {
  ...
  return (
    <div className="relative" ref={referencia}>
      <button
        ...
        disabled={pendente}
        className={cx(
          "group flex items-center gap-4 rounded-lg px-2 py-1.5 text-left transition-all hover:bg-primary/5 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-70",
        ...
        <div className="flex flex-col items-start">
          ...
          <div className="mt-0.5 flex items-center gap-2">
            <span className="text-[13px] font-bold text-slate-900">
              {pendente ? "aplicando..." : `Dificuldade ${valor}`}
            </span>
          </div>
        </div>
      ...
      {aberto ? (
        <div className="absolute right-0 top-full z-50 mt-2 w-64 origin-top-right rounded-xl border border-slate-200/60 bg-white/95 p-1 shadow-xl backdrop-blur-sm">
          ...
          <div className="space-y-0.5" role="listbox" aria-label="PoW global">
            {OPCOES_DIFICULDADE.map((opcao) => {
              ...
              return (
                ...
                >
                  <span className="text-[13px] font-semibold">Dificuldade {opcao}</span>
