...
class SupplyChainEvent:
    ...
    def validar_basico(self) -> bool:
        ...
        if not tipo_evento_valido(self.event_type):
            return False

        if not tipo_entidade_valido(self.entity_kind):
            return False

        if tipo_entidade_esperado_por_evento(self.event_type) != self.entity_kind:
            return False

        if not papel_ator_valido(self.actor_role):
            return False
