...
class Blockchain:
    ...
    def validar_bloco(self, block: Block, previous_block: Block) -> bool:
        """Valida o bloco usando a estrutura e o contexto da cadeia."""

        cadeia_base = self._obter_prefixo_cadeia_por_hash(previous_block.block_hash)
        if cadeia_base is None:
            return False

        return self._validar_bloco_em_cadeia(block, previous_block, cadeia_base)
    ...
    def _tentar_estender_candidata_existente(self, block: Block) -> list[Block] | None:
        ...
        for cadeia in self.cadeias_candidatas:
            ...
            if block.previous_hash != previous_block.block_hash:
                continue
            if not self._validar_bloco_em_cadeia(block, previous_block, cadeia):
                continue
    ...
    def _tentar_criar_fork_da_cadeia_ativa(self, block: Block) -> list[Block] | None:
        ...
        for index, previous_block in enumerate(self.chain[:-1]):
            if block.previous_hash != previous_block.block_hash:
                continue
            cadeia_base = self.chain[: index + 1]
            if not self._validar_bloco_em_cadeia(block, previous_block, cadeia_base):
                continue

            nova_cadeia = deepcopy(cadeia_base)
            nova_cadeia.append(deepcopy(block))
            return nova_cadeia

        return None

    def evento_pode_entrar_no_proximo_bloco(
        self,
        event: SupplyChainEvent,
        eventos_do_bloco: list[SupplyChainEvent] | None = None,
    ) -> bool:
        """Ajuda a selecionar eventos que ja conseguem ser minerados."""

        contexto_cadeia = self.validator.construir_contexto_cadeia(self.chain)
        if contexto_cadeia is None:
            return False

        contexto_eventos, contexto_consumidos = contexto_cadeia
        if eventos_do_bloco:
            contexto_parcial = self.validator.construir_contexto_eventos(
                eventos_do_bloco,
                contexto_eventos,
                contexto_consumidos,
            )
            if contexto_parcial is None:
                return False

            contexto_eventos, contexto_consumidos = contexto_parcial

        return self.validator.validar_evento_de_dominio(
            event,
            contexto_eventos,
            contexto_consumidos,
        )

    def _validar_bloco_em_cadeia(
        self,
        block: Block,
        previous_block: Block,
        cadeia_base: list[Block],
    ) -> bool:
        """Usa a cadeia correta como contexto para validar o bloco."""
        ...
        contexto_eventos, contexto_consumidos = contexto_cadeia
        return (
            self.validator.construir_contexto_eventos(
                block.events,
                contexto_eventos,
                contexto_consumidos,
            )
    ...
    def _obter_prefixo_cadeia_por_hash(self, block_hash: str) -> list[Block] | None:
        """Busca a cadeia conhecida que termina no hash informado."""

        for cadeia in [self.chain, *self.cadeias_candidatas]:
            for index, block in enumerate(cadeia):
                if block.block_hash == block_hash:
                    return deepcopy(cadeia[: index + 1])
    ...
    def event_can_enter_next_block(
        self,
        event: SupplyChainEvent,
        selected_events: list[SupplyChainEvent] | None = None,
    ) -> bool:
        """Alias de compatibilidade para `evento_pode_entrar_no_proximo_bloco`."""

        return self.evento_pode_entrar_no_proximo_bloco(event, selected_events)
